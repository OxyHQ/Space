/**
 * `pages` against a real PostgreSQL 17. Run by `bun run test:pgdb`.
 *
 * This file shares ONE database with every other `*.pgdb.test.ts`, so every
 * assertion that aggregates is scoped to a workspace or database id this file
 * created. It does not skip when Postgres is absent: `testDatabaseUrl()` throws,
 * because a suite that silently skips reports exactly what a passing suite
 * reports.
 */
...
describe('pages repository (real database)', () => {
  let db: TestDatabase;
  let workspaceId: string;

  beforeAll(async () => {
    db = await getTestDb();
    workspaceId = await seedWorkspace(db, testScope('pages-repo'));
  });
  ...
  describe('schema', () => {
    ...
    it('refuses a page in a workspace that does not exist', async () => {
      await expect(
        createPage(db, { workspaceId: uuidv7(), ownerId: 'u', order: 0 }),
    ...
    it('enforces the coverPosition bounds the Mongoose validator enforced', async () => {
      ...
      await expect(updatePage(db, withinBounds.id, { coverPosition: 0 })).resolves.toBeDefined();
      await expect(
        updatePage(db, withinBounds.id, { coverPosition: 100 }),
      ).resolves.toBeDefined();
      await expect(
        updatePage(db, withinBounds.id, { coverPosition: 101 }),
      ).rejects.toSatisfy(isCheckViolation);
      await expect(updatePage(db, withinBounds.id, { coverPosition: -1 })).rejects.toSatisfy(
        isCheckViolation,
      );
      // `between` rejects NaN, which the Mongoose min/max pair did not.
      await expect(
        updatePage(db, withinBounds.id, { coverPosition: Number.NaN }),
      ).rejects.toSatisfy(isCheckViolation);
      await deletePageTree(db, withinBounds.id);
    });
  });
  ...
  describe('createPage', () => {
    it('applies the defaults the Mongoose schema applied', async () => {
      ...
      const read = await findPageById(db, page.id);
  ...
  describe('listPages', () => {
    ...
    it('filters archived and favorited the way the query parameters do', async () => {
      ...
      await updatePage(db, trashed.id, { archived: true });
      await updatePage(db, starred.id, { favorited: true });

      const active = await listPages(db, { workspaceId, archived: false });
      expect(active.map((page) => page.title).sort()).toEqual(['live', 'starred']);

      const trash = await listPages(db, { workspaceId, archived: true });
      ...
      const favorites = await listPages(db, { workspaceId, favoritedOnly: true });
      expect(favorites.map((page) => page.title)).toEqual(['starred']);

      const everything = await listPages(db, { workspaceId });
      expect(everything).toHaveLength(3);

      for (const page of [live, trashed, starred]) await deletePageTree(db, page.id);
    });
  });
  ...
  describe('findPagesByIds', () => {
    it('scopes by workspace and by archived, and returns [] for no ids', async () => {
      ...
      await updatePage(db, archived.id, { archived: true });

      const otherWorkspaceId = await seedWorkspace(db, testScope('pages-other'));
      ...
      expect((await findPagesByIds(db, ids, { workspaceId })).map((p) => p.id).sort()).toEqual(
        [mine.id, archived.id].sort(),
      );
      expect((await findPagesByIds(db, ids, { archived: false })).map((p) => p.id).sort()).toEqual(
        [mine.id, theirs.id].sort(),
      );
      expect(await findPagesByIds(db, [])).toEqual([]);

      await deletePageTree(db, mine.id);
      await deletePageTree(db, archived.id);
      await deletePageTree(db, theirs.id);
  ...
  describe('nextSiblingOrder', () => {
    it('is 0 with no sibling, max+1 otherwise, and scoped to the parent', async () => {
      expect(await nextSiblingOrder(db, { workspaceId, parentId: null })).toBe(0);

      const root = await seedPage({ order: 0 });
      const parent = await seedPage({ order: 7 });
      expect(await nextSiblingOrder(db, { workspaceId, parentId: null })).toBe(8);
      expect(await nextSiblingOrder(db, { workspaceId, parentId: parent.id })).toBe(0);

      await seedPage({ parentId: parent.id, order: 2.25 });
      const next = await nextSiblingOrder(db, { workspaceId, parentId: parent.id });
      ...
      await deletePageTree(db, root.id);
      await deletePageTree(db, parent.id);
    });
  });
  ...
  describe('updatePage', () => {
    it('writes only defined keys, and treats null as a value rather than an omission', async () => {
      ...
      const renamed = await updatePage(db, page.id, { title: 'after', icon: undefined });
      ...
      const cleared = await updatePage(db, page.id, { icon: null });
    ...
    it('bumps updatedAt and leaves createdAt alone', async () => {
      ...
      const updated = await updatePage(db, page.id, { title: 'touched' });
    ...
    it('merges properties rather than replacing them', async () => {
      ...
      const merged = await updatePage(db, page.id, {
    ...
    it('answers undefined for an unknown id and refuses an empty patch', async () => {
      expect(await updatePage(db, uuidv7(), { title: 'nobody' })).toBeUndefined();
      const page = await seedPage();
      await expect(updatePage(db, page.id, {})).rejects.toThrow(/no fields to write/u);
      await deletePageTree(db, page.id);
    });
  });
  ...
  describe('deletePageTree', () => {
    it('removes the page, its descendants and their blocks, and counts the pages', async () => {
      ...
      expect(await deletePageTree(db, root.id)).toBe(3);
      expect(await findPageById(db, root.id)).toBeUndefined();
      expect(await findPageById(db, child.id)).toBeUndefined();
      expect(await findPageById(db, grandchild.id)).toBeUndefined();

      // Scoped to the four pages this test created. Unscoped, this reads every
      // block a sibling `*.pgdb.test.ts` has seeded into the one shared
      // database and fails for a reason unrelated to the cascade.
      const survivingBlocks = await db
        .select()
        .from(blocks)
        .where(inArray(blocks.pageId, [root.id, child.id, grandchild.id, bystander.id]));
      ...
      const untouched = await findPageById(db, bystanderChild.id);
      expect(untouched?.parentId).toBe(bystander.id);

      await deletePageTree(db, bystander.id);
    });
    ...
    it('terminates on a parent cycle', async () => {
      ...
      await updatePage(db, a.id, { parentId: b.id });

      expect(await deletePageTree(db, a.id)).toBe(2);
      expect(await findPageById(db, b.id)).toBeUndefined();
    });

    it('is 0 for an id nothing holds', async () => {
      expect(await deletePageTree(db, uuidv7())).toBe(0);
    });
  });
  ...
  describe('deleteArchivedPages', () => {
    ...
    it('keeps a live child of an archived page, detached rather than destroyed', async () => {
      ...
      await updatePage(db, archivedParent.id, { archived: true });
      await updatePage(db, archivedChild.id, { archived: true });

      expect(await deleteArchivedPages(db, workspaceId)).toBe(2);

      const survivor = await findPageById(db, liveChild.id);
      expect(survivor).toBeDefined();
      expect(survivor?.parentId).toBeNull();
      expect(await findPageById(db, archivedChild.id)).toBeUndefined();

      await deletePageTree(db, liveChild.id);
    });
  });
  ...
  describe('findPageAncestry', () => {
    it('returns the chain root-first and stops at the depth bound', async () => {
      ...
      expect(await findPageAncestry(db, leaf.id)).toEqual([
        { id: root.id, title: 'root', icon: 'home' },
        { id: middle.id, title: 'middle', icon: null },
        { id: leaf.id, title: 'leaf', icon: null },
      ]);
      expect(await findPageAncestry(db, root.id)).toEqual([
        { id: root.id, title: 'root', icon: 'home' },
      ]);
      expect(await findPageAncestry(db, uuidv7())).toEqual([]);
  ...
  describe('database rows', () => {
    it('lists, orders, unsets a property and deletes, scoped to one database', async () => {
      ...
      await updatePage(db, archivedRow.id, { archived: true });
      const elsewhere = await seedPage({ databaseId: otherDatabaseId, order: 100 });

      expect((await listDatabaseRows(db, databaseId)).length).toBe(3);
      expect(
        (await listDatabaseRows(db, databaseId, { archived: false })).map((row) => row.id).sort(),
      ).toEqual([first.id, second.id].sort());

      const nextOrder = await nextDatabaseRowOrder(db, databaseId);
      expect(typeof nextOrder).toBe('number');
      expect(nextOrder).toBe(10);

      const matched = await removePropertyFromDatabaseRows(db, databaseId, 'p1');
      expect(matched).toBe(3);
      expect((await findPageById(db, first.id))?.properties).toEqual({ p2: { number: 1 } });
      expect((await findPageById(db, second.id))?.properties).toEqual({});
      // Scoped: the other database's row is untouched.
      expect(await nextDatabaseRowOrder(db, otherDatabaseId)).toBe(101);

      expect(await deleteDatabaseRows(db, databaseId)).toBe(3);
      expect(await listDatabaseRows(db, databaseId)).toEqual([]);
      expect(await findPageById(db, elsewhere.id)).toBeDefined();

      await deleteDatabaseRows(db, otherDatabaseId);
    });
  });
});
