# Oxy Station Developers Portal

> Note: This portal originally targeted the legacy Clarity AI chat product. It remains documented while the API endpoints are still mounted, but the chat-product features are not part of the Oxy Station user-facing surface.
...
```bash
curl -X POST https://api.station.oxy.so/internal/trigger \
...
3. **Use the API Key**
   ```bash
   curl -X POST https://api.station.oxy.so/v1/chat/completions \
...
```javascript
// Using fetch
const response = await fetch('https://api.station.oxy.so/v1/chat/completions', {
  method: 'POST',
  headers: {
